...
from models import (
    ...
    WorkAttributeOption, Material, MaterialRecalculationLog, MaterialSubstitution,
)
...
def admin_list_materials(
    ...
    """管理员材料列表：状态、影响配方数、编辑、重算、删除。"""
    q = db.query(Material).filter(Material.is_active.is_(True))
    if status:
        q = q.filter(Material.status == status)
    if search:
        keyword = f"%{search.strip()}%"
        q = q.filter(or_(Material.name.ilike(keyword), Material.name_en.ilike(keyword)))
    total = q.count()
    items = q.order_by(Material.id.desc()).offset((page - 1) * page_size).limit(page_size).all()
    data = []
    for material in items:
        affected_count = db.query(func.count(func.distinct(RecipeIngredient.recipe_id))).filter(
...
def admin_update_material(
    ...
    conflict = find_material_name_conflict(
        db, name=final_name, name_en=final_name_en, exclude_id=material.id,
    )
    if conflict:
        raise HTTPException(
            status_code=409,
            detail=f"中英文材料名组合已存在（ID {conflict.id}，忽略空白）",
        )
    ...
    db.commit()
    db.refresh(material)
    return catalog_payload(material)


@router.post("/materials/{material_id}/recalculate")
@router.post("/materials/{material_id}/approve-and-recalculate")
def admin_recalculate_material(
    material_id: int,
    body: dict | None = None,
    db: Session = Depends(get_db),
    ...
    submitted_data = (body or {}).get("data") or {}
    if submitted_data:
        cleaned = clean_molecule_data(submitted_data, partial=True)
        final_name = cleaned.get("name", material.name)
        final_name_en = cleaned.get("name_en", material.name_en or "")
        conflict = find_material_name_conflict(
            db, name=final_name, name_en=final_name_en, exclude_id=material.id,
    ...
    if not any(getattr(material, field, None) not in (None, 0, 0.0) for field in oxide_fields):
    ...
    result = recalculate_material_recipes(db, material, admin_user_id=admin.id)
    material = db.query(Material).filter(Material.id == material_id).first()
    ...
    db.commit()
    return {
        "message": "审核及重算完成" if result["failed"] == 0 else "部分配方重算失败",
        "result": result,
...
def admin_affected_recipes(
    material_id: int, db: Session = Depends(get_db), _=Depends(verify_admin),
):
    rows = db.query(Recipe).join(
    ...
    return [
        {"id": row.id, "recipe_no": row.recipe_no, "title": row.title, "user_id": row.user_id}
        for row in rows
    ]


@router.get("/material-recalculation-logs")
def admin_material_recalculation_logs(
    material_id: int | None = Query(None),
    db: Session = Depends(get_db),
    _=Depends(verify_admin),
):
    query = db.query(MaterialRecalculationLog)
    if material_id is not None:
        query = query.filter(MaterialRecalculationLog.material_id == material_id)
    rows = query.order_by(MaterialRecalculationLog.id.desc()).limit(200).all()
    return [{
        "id": row.id,
        "material_id": row.material_id,
        "admin_id": row.admin_id,
        "affected_recipe_count": row.affected_recipe_count,
        "success_count": row.success_count,
        "failed_count": row.failed_count,
        "recipe_ids": json.loads(row.recipe_ids_json or "[]"),
...
def admin_delete_material(
    ...
    if affected:
        raise HTTPException(status_code=409, detail=f"该材料影响 {len(affected)} 个配方，不能删除")
    db.query(MaterialSubstitution).filter(or_(
